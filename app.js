...
function fetchData(date) {
  const url = `http://report.amap.com/cityTravel/inAndOutCity.do?adcode=100000&dt=${date}&willReal=WILL&inOut=IN&size=500`;
    axios.get(url, {
      timeout: 5000,
      headers: {
...
app.get('/', (req, res) => {
  const FROM_DATE = new Date("2020-08-06"); // dinh dang yyyy-mm-dd
  ...
  asynFetchData(customDayArray, WAITING_TIME);
});
// chay = npm run start1 
