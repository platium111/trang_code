...
function fetchData(date) {
  const url = `https://report.amap.com/cityTravel/line.do?adcode=100000&dt=${date}&willReal=WILL&size=130000`;
    axios.get(url, {
      timeout: 50000,
      headers: {
...
app.get('/', (req, res) => {
  const FROM_DATE = new Date("2021-01-09"); // dinh dang yyyy-mm-dd
  const END_DATE = new Date("2021-07-21") || new Date();
  const WAITING_TIME = 40000; // cho 8 giay moi lan request
  // * get date array
  ...
  const customDayArray = ["2020-08-05", "2020-08-31", "2020-09-05", "2020-09-12", "2020-10-26", "2020-11-19", "2020-12-02",
    "2020-12-16"]
